...
import jwt, psycopg2, datetime, random, cloudinary.uploader, traceback
...
def get_conn():
    return psycopg2.connect("postgresql://neondb_owner:npg_UATC3pfibMd6@ep-cold-cake-abnyap5j.eu-west-2.aws.neon.tech/neondb?sslmode=require")
...
def extract_cloudinary_public_id(url):
    """Safely extracts public_id from Cloudinary URL"""
    if not url or "cloudinary" not in url: return None
    try:
        # Format: .../upload/[version]/[path/]filename.ext
...
def delete_cloudinary_asset(url, resource_type="image"):
    """Deletes an asset from Cloudinary by URL"""
    ...
    try:
        ...
        print(f"🗑️ Deleted from Cloudinary: {public_id} ({resource_type})")
        return True
    except Exception as e:
    ...
    try:
        ...
        while True:
            ...
            if not cursor.fetchone():
                break
        cursor.execute("INSERT INTO users (name, email, phone, username, password, profile_picture) VALUES (%s, %s, %s, %s, %s, %s)", (name, email, phone, username, password, "unknown"))
        conn.commit()
        ...
        conn.rollback()
        return jsonify({"msg": "error", "debug": str(e)}), 500
    finally:
        cursor.close()
        conn.close()
    ...
    try:
        ...
        if user:
            ...
            while True:
                ...
                if not cursor.fetchone():
                    break
            cursor.execute("INSERT INTO users (name, email, phone, username, password, profile_picture) VALUES (%s, %s, NULL, %s, %s, %s)", (name, email, username, "google_auth", picture))
            conn.commit()
        token = jwt.encode({"user": username, "exp": datetime.datetime.utcnow() + datetime.timedelta(days=30)}, app.secret_key, algorithm="HS256")
        return jsonify({"msg": "success", "token": token, "username": username, "profile_picture": picture})
    except Exception as e:
        conn.rollback()
        return jsonify({"msg": "server_error", "debug": str(e)}), 500
    finally:
        cursor.close()
        conn.close()
...
def add_post():
    ...
    if not token: return jsonify({"msg": "no token"}), 401
    ...
    try:
        ...
        return jsonify({"msg": "invalid_token", "debug": str(e)}), 401
    
    user_val = data.get("user_id") or data.get("user")
    if not user_val: return jsonify({"msg": "no_user_identifier"}), 401
    
    # Resolve user_id
    if isinstance(user_val, str):
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT id FROM users WHERE username = %s", (user_val,))
        row = cursor.fetchone(); cursor.close(); conn.close()
        if not row: return jsonify({"msg": "user_not_found"}), 401
        user_id = row[0]
    else:
        user_id = user_val

    content = request.form.get("content")
    image_url = request.form.get("image_url")  # From frontend
    video_url = request.form.get("video_url")  # ✅ From frontend

    print(f"📥 Received: content_len={len(content) if content else 0}, image_url={image_url}, video_url={video_url}")

    conn = get_conn(); cursor = conn.cursor()
    try:
        # ✅ Insert with video column
        cursor.execute(
            "INSERT INTO posts (user_id, content, image, video) VALUES (%s, %s, %s, %s)",
            (user_id, content, image_url, video_url)
        )
        conn.commit()
        print(f"✅ Post saved to DB: user={user_id}, video_url={video_url}")
        return jsonify({"msg": "post_created"})
    except Exception as e:
        conn.rollback()
        print(f"❌ DB insert error: {e}")
        import traceback; traceback.print_exc()
        return jsonify({"msg": "db_error", "debug": str(e)}), 500
    finally:
...
def get_posts():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT posts.id, posts.content, posts.image, posts.video,  -- ✅ Added posts.video
...
def like_post():
    ...
    if isinstance(user_val, str):
        ...
        conn.close()
        if not row:
            return jsonify({"msg": "user_not_found"}), 401
        user_id = row[0]
    else:
        user_id = user_val
    req = request.json
...
def follow():
    ...
    if not token: return jsonify({"msg": "no token"}), 401
    token = token.replace("Bearer ", "").strip()
    try: data = jwt.decode(token, app.secret_key, algorithms=["HS256"])
    except: return jsonify({"msg": "invalid_token"}), 401
    user_val = data.get("user_id") or data.get("user")
    if not user_val: return jsonify({"msg": "no_user_identifier"}), 401
    target = request.json.get("username")
    if not target or target == user_val: return jsonify({"msg": "invalid_target"}), 400
    ...
    try:
        ...
        cur.execute("SELECT id FROM users WHERE username = %s", (user_val,))
        u_row = cur.fetchone()
        user_id = u_row[0] if u_row else user_val
        ...
        if already_following:
            ...
            if cur.fetchone():
                return jsonify({"msg": "unfollowed", "status": "following"}), 200
            else:
                return jsonify({"msg": "unfollowed", "status": "none"}), 200
        else:
            cur.execute("INSERT INTO follows (follower_id, following_id) VALUES (%s, %s)", (user_id, target_id))
            conn.commit()
            cur.execute("SELECT id FROM follows WHERE follower_id=%s AND following_id=%s", (target_id, user_id))
            if cur.fetchone():
                return jsonify({"msg": "friends", "status": "friends"}), 200
            else:
                return jsonify({"msg": "followed", "status": "following"}), 200
    except Exception as e:
        conn.rollback()
        return jsonify({"msg": "error", "debug": str(e)}), 500
    finally:
...
def get_friends():
    ...
    try:
        ...
        friends = [{"id": r[0], "username": r[1], "name": r[2], "profile_picture": r[3], "is_online": r[4]} for r in cur.fetchall()]
...
def get_my_follows():
    ...
    try:
        ...
        print(f"Get my follows error: {e}")
        return jsonify({"msg": "server_error", "debug": str(e)}), 500
    finally:
        cur.close()
        conn.close()
...
def get_comments(post_id):
    ...
    try:
        ...
        """, (post_id,))
        rows = cur.fetchall()
        return jsonify([{"id": r[0], "content": r[1], "created_at": str(r[2]), "username": r[3], "name": r[4], "profile_picture": r[5]} for r in rows])
    finally:
        cur.close()
        conn.close()
...
def add_comment():
    ...
    try:
        if isinstance(user_val, str):
            cur.execute("SELECT id FROM users WHERE username = %s", (user_val,))
            row = cur.fetchone()
            user_id = row[0] if row else user_val
        else:
            user_id = user_val
...
def delete_post(post_id):
    ...
    try:
        # 1. Fetch post media URLs & verify ownership
        cur.execute("SELECT user_id, image, video FROM posts WHERE id = %s", (post_id,))
        row = cur.fetchone()
        if not row: return jsonify({"msg": "post_not_found"}), 404
        post_owner_id, image_url, video_url = row

        # Resolve current user ID
        user_id = user_val if isinstance(user_val, int) else None
        if not user_id:
            cur.execute("SELECT id FROM users WHERE username = %s", (user_val,))
            u_row = cur.fetchone()
            user_id = u_row[0] if u_row else user_val
        ...
        # 2. ✅ Delete media from Cloudinary FIRST
        if image_url: delete_cloudinary_asset(image_url, "image")
        if video_url: delete_cloudinary_asset(video_url, "video")
...
def get_my_info():
    ...
    token = token.replace("Bearer ", "").strip()
    try: data = jwt.decode(token, app.secret_key, algorithms=["HS256"])
    except Exception as e: return jsonify({"msg": "invalid_token", "debug": str(e)}), 401
    user_val = data.get("user_id") or data.get("user")
    if not user_val: return jsonify({"msg": "no_user_identifier"}), 401
...
def update_profile():
    ...
    try:
        ...
        print(new_email,"/",new_username,"/",new_name,"/",new_password,"/",old_password)
        ...
        if img and img.filename != "":
            try:
                ...
                return jsonify({"msg": "upload_failed"}), 500
        ...
        if new_password:
            ...
            if len(new_password) < 6: return jsonify({"msg": "password_too_short"}), 400
...
def get_user_profile(username):
    ...
    try:
        ...
        # ✅ Added u.profile_picture to the SELECT
        cur.execute("""
...
def repost_post():
    ...
    req = request.json; post_id = req.get("post_id")
...
def edit_comment(comment_id):
    ...
    if not req_data or "content" not in req_data:
        return jsonify({"msg": "missing_content"}), 400
    content = req_data["content"].strip()
    if not content: return jsonify({"msg": "empty_content"}), 400

    conn = get_conn(); cur = conn.cursor()
    try:
        cur.execute("SELECT post_id, user_id FROM comments WHERE id = %s", (comment_id,))
        row = cur.fetchone()
        if not row: return jsonify({"msg": "comment_not_found"}), 404
        ...
        if user_id != comment_owner: return jsonify({"msg": "unauthorized"}), 403
...
def delete_comment(comment_id):
    ...
    try:
        ...
        if isinstance(user_val, int): user_id = user_val
        else:
            cur.execute("SELECT id FROM users WHERE username = %s", (user_val,))
            u_row = cur.fetchone()
            user_id = u_row[0] if u_row else user_val
        ...
        cur.close(); conn.close()
# ===== MESSAGES ROUTES (PASTE AT GLOBAL SCOPE) =====
...
def search_users():
    ...
    try:
        ...
        users = [{"username": r[0], "name": r[1], "profile_picture": r[2]} for r in cur.fetchall()]
...
def get_friends_list():
    ...
    try:
        ...
        if user_id: update_last_seen(user_id)
...
def get_messages(other_username):
    ...
    try:
        ...
        if not other_user: return jsonify({"msg": "user_not_found"}), 404
        ...
        for row in cur.fetchall():
            ...
            # 6. Fetch Reactions for this message
            ...
            # ✅ Find which emojis CURRENT USER reacted with
            ...
            messages.append({
                ...
                "user_reactions": user_reactions  # ✅ Send to frontend
            })
        ...
        cur.close(); conn.close()
@app.route("/send_message", methods=["POST"])  # ✅ MUST be a LIST ["POST"]
def send_message():
    token = request.headers.get("Authorization")
    if not token: return jsonify({"msg": "no token"}), 401
    token = token.replace("Bearer ", "").strip()
    ...
    content = request.json.get("content", "").strip()
    if not receiver_username or not content: return jsonify({"msg": "missing_fields"}), 400

    conn = get_conn(); cur = conn.cursor()
    try:
        sender_id = user_val if isinstance(user_val, int) else None
        if not sender_id:
            cur.execute("SELECT id FROM users WHERE username = %s", (user_val,))
            row = cur.fetchone()
            sender_id = row[0] if row else user_val
...
def delete_message(message_id):
    ...
    try:
        ...
        row = cur.fetchone()
        if not row: return jsonify({"msg": "message_not_found"}), 404
...
def add_reaction():
    ...
    try:
        ...
        # ✅ NEW: Prevent reacting to your own messages
        cur.execute("SELECT sender_id FROM messages WHERE id = %s", (message_id,))
...
if __name__ == "__main__":
    ...
    cursor.execute("""
        ...
        ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_picture TEXT DEFAULT 'unknown';
